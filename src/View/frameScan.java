...
public class frameScan {
	...
	private void initialize() {
		...
		*/
		...
		//Types 
		//Select type/s of files to look for
		...
		path = (String) cbDir.getSelectedItem();
		...
		Object[][] files = {}; 
		JTable tableFiles = new JTable(new DefaultTableModel(files, columnNames));
		...
		//Start a scan with the selected parameters
		JButton btnNewScan = new JButton("New Scan");
		btnNewScan.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent arg0) {
				Directory filesFound = new Directory(path);
				//Get the list of files from Directory
				ArrayList<UserFile> userFiles = filesFound.getFileList();
				//Add new items to table
				DefaultTableModel tableModel = (DefaultTableModel) tableFiles.getModel();
				for (UserFile newFiles : userFiles) {
					String fName = newFiles.getFileName();
					/*
					 * Need to add fileType getter or use the type selected on scanFrame
					 *String fType = newFiles.getType();
					*/
					String lastAccessed= new String();
					 try {
						lastAccessed = newFiles.getLastAccessedDate();
						
					} catch (IOException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
					 /*Not sure where to get the directory of the file
					 *String location = userFiles.getDirectory;
					*/
					 /*Not sure where to get the directory of the file
						 *String location = userFiles.getDirectory;
						*/
					tableModel.addRow(new Object[]{fName,path,selectedType.get(0),null,lastAccessed});
				}
			}
		});
		btnNewScan.setBounds(335, 62, 115, 29);
		frame.getContentPane().add(btnNewScan);
